...
export function getInvestors(projectId: string) {
  return request<{ investors: InvestorRecord[] }>(`/api/investors/${projectId}`);
}
...
export function checkReplies(projectId: string) {
  return request<{ updated: InvestorRecord[] }>(`/api/investors/${projectId}/check-replies`);
}
