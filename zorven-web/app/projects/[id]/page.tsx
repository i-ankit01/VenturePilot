...
import {
  Cpu, BarChart3, Swords, Package,
  Palette, DollarSign, Rocket, Presentation,
...
function PanelSkeleton({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="mt-1 text-[12px] text-muted-foreground">Results will appear here automatically</p>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-muted-foreground">
        <span className="font-medium text-foreground" style={{ fontFamily: "'DM Mono', monospace" }}>{label}</span> hasn't started yet
      </p>
      <p className="mt-1 text-[12px] text-muted-foreground/60">Agents run sequentially — check back soon</p>
    </div>
  );
}
...
export default function WorkspacePage({ params }: PageProps) {
  ...
  const { data, status, error, completedAgents } = usePipelineProgress(jobId);
    ...
    <AppShell>
      <div className="flex h-full flex-col">
        ...
        <div className="flex items-center justify-between border-b border-border/60 bg-card/50 px-6 py-3 backdrop-blur-sm">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/50 mb-0.5" style={{ fontFamily: "'DM Mono', monospace" }}>
              Job ID
            </p>
            <p className="text-[12px] text-muted-foreground" style={{ fontFamily: "'DM Mono', monospace" }}>
              {jobId}
            </p>
          </div>
          <div className="flex items-center gap-2">
            {isRunning && (
              <span className="flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[11px] font-semibold text-primary" style={{ fontFamily: "'DM Mono', monospace" }}>
                <Loader2 className="h-3 w-3 animate-spin" />
                Pipeline running
              </span>
            )}
            {status === "done" && (
              <span className="flex items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1 text-[11px] font-semibold text-emerald-400" style={{ fontFamily: "'DM Mono', monospace" }}>
                ✓ Complete
              </span>
            )}
          </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  <tab.icon
                    className={cn("h-3.5 w-3.5 shrink-0", isActive ? "text-primary" : "text-muted-foreground/50")}
                  ...
                  <span className="ml-auto">
                    {isDone && <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_4px_#34d399] inline-block" />}
                    {isRunningNow && !isDone && <Loader2 className="h-3 w-3 animate-spin text-primary" />}
                    {!isDone && !isRunningNow && <span className="h-1.5 w-1.5 rounded-full bg-border inline-block" />}
                  </span>
                </button>
              );
            })}
